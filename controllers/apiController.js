    ...
    exec(
      ...
      (err) => {
        ...
        if (isset(req.body.email))
          emailManager.sendProjectFinished(
            req.body.email,
  ...
  projectManager.load(req.params.projectID, "w").then(
    ([document, , release]) => {
      ...
      projectManager.save(req.params.projectID, root.outerHTML, release).then(
        () => res.json({ msg: "리소스가 성공적으로 제거되었습니다." }),
        (err) => next(err)
      );
    },
    (err) => fileErr(err, res)
  ...
  projectManager.load(req.params.projectID, "w").then(
    ([document, , release]) => {
      ...
      // Complex + Complex
        ...
        const filters = multitrackB.parentElement.getElementsByTagName(
          "filter"
        );
    ...
    ([document, , release]) => {
      ...
      if (targetElement.entries.length === 0) {
        ...
        if (blank !== null) {
          blank.setAttribute(
            "length",
            timeManager.subDuration(blank.getAttribute("length"), itemDuration)
          );
