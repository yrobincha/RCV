...
export default {
  ...
  getItemAtTime(document, track, time) {
    ...
    for (let entry of entries) {
      // Blank
      if (entry.tagName === "blank") {
        currentTime = timeManager.addDuration(
          currentTime,
          entry.getAttribute("length")
        );
      }
      // Simple entry
      else if (new RegExp(/^producer/).test(entry.getAttribute("producer"))) {
        currentTime = timeManager.addDuration(
          currentTime,
          this.getDuration(entry, document).time
        );
      }
      // Container of entries
      else {
        ...
        currentTime = timeManager.addDuration(
          currentTime,
          this.getDuration(multitrack, document).time
        );
      }

      if (time < currentTime) {
        // Time is somewhere inside element
        return {
          endTime: currentTime,
          entries: [entry],
        };
  ...
  isUsedInTractor(track) {
    ...
    for (let transition of transitions) {
      if (transition.getAttribute("a_track") === trackIndex.toString())
        return true;
      if (transition.getAttribute("b_track") === trackIndex.toString())
        return true;
    }
  ...
  getDuration(element, document) {
    ...
    if (element.tagName === "multitrack") {
      ...
      const playlist = document.getElementById(
        lastTrack.getAttribute("producer")
      );
      duration.time = playlist.childNodes.item(0).getAttribute("length");
      if (duration.time === null)
        duration.time = this.getDuration(
          playlist.childNodes.item(playlist.childElementCount - 1),
          document
        ).time;
      else
        duration.time = timeManager.addDuration(
          duration.time,
          this.getDuration(
            playlist.childNodes.item(playlist.childElementCount - 1),
            document
      ...
      if (duration.out === null) {
        const producer = document.getElementById(
          element.getAttribute("producer")
        );
        const properties = producer.getElementsByTagName("property");
        for (let property of properties) {
          if (property.getAttribute("name") === "length")
            duration.out = property.innerHTML;
        }
      }

      if (duration.in > duration.out)
        throw `Attribute in is greater than out: ${element.outerHTML}`;
  ...
  entryToPlaylist(item, document) {
    ...
    const lastID =
      lastPlaylist === null ? -1 : lastPlaylist.id.match(/playlist(\d+)/)[1];
    ...
    lastProducer.parentElement.insertBefore(
      newPlaylist,
      lastProducer.nextSibling
    );
    return newPlaylist;
  },
  ...
  appendPlaylistToMultitrack(
    ...
    const duration = timeManager.subDuration(
      this.getDuration(multitrack, document).time,
      overlapping
    );
    ...
    transitionElement.setAttribute(
      "out",
      timeManager.addDuration(duration, overlapping)
    );
    transitionElement.setAttribute(
      "a_track",
      (multitrack.childElementCount - 2).toString()
    );
    transitionElement.setAttribute(
      "b_track",
      (multitrack.childElementCount - 1).toString()
    );
  ...
  createTractor(document) {
    ...
    const lastID =
      lastTractor === null ? -1 : lastTractor.id.match(/tractor(\d+)/)[1];
